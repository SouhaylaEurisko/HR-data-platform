...
# ──────────────────────────────────────────────
# One-shot import (backward compatible)
# ──────────────────────────────────────────────

async def import_workbook_oneshot(
    workbook,
    contents: bytes,
    filename: str,
    sheet_names: List[str],
    ...
    """
    One-shot import that auto-accepts all high-confidence mappings.
    Backward-compatible with the old single-endpoint flow.
    """
    analysis = await analyze_workbook(
        workbook, contents, filename, sheet_names, org_id, user_id, db,
    )

    # Build confirmed mappings from matched + suggested columns
    confirmed: Dict[str, str] = {}
    for m in analysis["matched_columns"]:
        confirmed[m["excel_header"].strip().lower()] = m["db_column"]
    for m in analysis["suggested_columns"]:
        confirmed[m["excel_header"].strip().lower()] = m["db_column"]

    # Auto-create custom fields for unmatched columns
    new_cfs = [
        {"header": m["excel_header"], "label": m["excel_header"]}
        for m in analysis["unmatched_columns"]
    ]

    return confirm_and_import(
        session_id=analysis["session_id"],
        confirmed_mappings=confirmed,
        new_custom_fields=new_cfs,
        skip_columns=[],
