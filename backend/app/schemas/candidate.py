"""Candidate and HR stage comment HTTP schemas."""
...
from ..dtos.hr_stage_comments import (
    HR_STAGE_COMMENT_MAX_TEXT_LEN,
    HrStageCommentEntryRead,
    HrStageCommentsRead,
    StageKeyLiteral,
)
from ..models.enums import ApplicationStatus, RelocationOpenness, TransportationAvailability
...
class CandidateHrStageCommentCreate(BaseModel):
    ...
    def _trim_and_cap(cls, v: Any) -> str:
        if v is None:
            return ""
        s = str(v).strip()
        return s[:HR_STAGE_COMMENT_MAX_TEXT_LEN] if len(s) > HR_STAGE_COMMENT_MAX_TEXT_LEN else s
